<template>
  <div class="container">
    <!-- <mt-header title="我的喜爱 "></mt-header> -->
    <p class="title">我的喜爱</p>
    <div id="stars"></div>
    <div id="stars2"></div>
    <div id="stars3"></div>
    <div id="title"></div>
    <router-link :to="'/RandomListen/'+showID+'/'+skipShowID">
     <div v-for="(item,index) in likeLists" :key="index" class="skipshow" > 
       <span class="showdesc">
         <img :src="item.album.cover.urls[0].url" alt="" class="imgs">
         {{item.show.name}}
         </span>      
    </div> 
      </router-link>
...
export default {
  ...
  mounted() {
   // console.log(this.$store.state.likeShows);
    this.getLikeShows();
  },
  methods: {
    getLikeShows(){
       this.$store.state.likeShows.forEach(item=>{
          this.showID=item.showID;
         this.skipShowID=item.skipShowID;
         // this.showID=showID;
        // this.skipShowID=skipShowID
       skipShowsList(item.showID,item.skipShowID).then(data=>{
         //console.log(data.data)
         //console.log(data.data.showList)
         if (data) {      
           if (data.data && data.data.showList){
                 for (let i in data.data.showList) {
              this.likeLists.push(data.data.showList[i]);
            }
           }else{
             return;
           }
